...
import GlobeAnimation from "./EnhancedGlobeAnimation";
...
export const PreviewPanel = forwardRef<PreviewPanelRef, PreviewPanelProps>(
  (
      ...
      switch (transitionType) {
        ...
        case "slideRight":
          ...
          ctx.drawImage(
            ...
            xRight,
            yRight,
            currentImage.width * scaleRight,
            currentImage.height * scaleRight,
          );
          break;
        ...
        case "zoomOut":
          ...
          ctx.drawImage(
            ...
            xZoomOut,
            yZoomOut,
            currentImage.width * zoomOutScale,
            currentImage.height * zoomOutScale,
          );
          ctx.globalAlpha = 1;
          break;
    ...
    const seekToPosition = (targetTime: number) => {
      ...
      console.log(
        `⏩ Seeking to ${targetTime.toFixed(1)}s (Slide ${targetIndex + 1}, Progress ${targetProgress.toFixed(1)}s)`,
      );
    };
    ...
    const currentItem = items[currentIndex];
    const progressPercentage = currentItem && items.length > 0 ? (progress / currentItem.duration) * 100 : 0;
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm text-muted-foreground">
                ...
                <span className="font-mono">
                  {progress.toFixed(1)}s / {currentItem?.duration.toFixed(1) || 0}s
                </span>
              </div>
              ...
              <div className="flex items-center justify-center gap-2">
                <Button onClick={handlePlayPause} size="lg" className="gap-2" disabled={showGlobeAnimation}>
                  {isPlaying ? (
                    <>
              ...
              {audioFile && (
                <div className="text-center text-sm text-muted-foreground">🎵 Background music: {audioFile.name}</div>
              )}
