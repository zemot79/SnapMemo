...
const Index = () => {
  ...
  const handleTitleNext = useCallback(
    (title: string, description: string, location: string, dateFrom: string, dateTo: string) => {
      setVideoTitle(title);
  ...
  const createTitleCard = useCallback(
    async (firstImage: File, title: string, description: string, date: string): Promise<MediaItem> => {
    ...
    async (files: File[]) => {
      ...
      if (shouldCreateTitleCard) {
        ...
        setMediaItems((prev) => {
          ...
          console.log(
            "🎬 Updated mediaItems with title card:",
            result.map((i) => ({ id: i.id, type: i.type })),
          );
          return result;
        });
  ...
  const handleKenBurnsChange = useCallback(
    (id: string, kenBurns: { enabled: boolean; effect: "zoomIn" | "zoomOut" | "panLeft" | "panRight" }) => {
  ...
  const handleExport = useCallback(
    (settings: ExportSettings): number => {
        ...
        <main className="pb-8">
          ...
          {currentStep === 2 && (
            <div className="max-w-6xl mx-auto space-y-6">
              <div className="text-center space-y-2 mb-6">
                ...
                <p className="text-muted-foreground">Add images and mark the main focal point</p>
                {getImageCount() > 0 && (
                  <p className="text-sm text-tertiary font-medium">{getImageCount()} images added</p>
                )}
              </div>
              ...
              {getImageCount() > 0 && (
                <div className="bg-card rounded-lg border border-border p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold">Click images to set focal points (max 2)</h3>
                    <p className="text-xs text-muted-foreground">💡 Point 1: Focal zoom | Point 2: Ken Burns target</p>
                  </div>
                  <ImageEditor
          ...
          {currentStep === 3 && (
            <div className="max-w-6xl mx-auto space-y-6">
              <div className="text-center space-y-2 mb-6">
                ...
                {getVideoCount() > 0 && (
                  <p className="text-sm text-tertiary font-medium">{getVideoCount()} videos added</p>
                )}
              </div>
              ...
              {getVideoCount() > 0 && (
                <div className="bg-card rounded-lg border border-border p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold">Set the time ranges for important clips</h3>
                    <p className="text-xs text-muted-foreground">💡 Drag elements to reorder</p>
                  </div>
                  <VideoEditor
                    ...
                    onReorder={(from, to) => {
                      ...
                      const allItems = [...mediaItems];
                      const videoIndices = allItems
                        .map((item, idx) => (item.type === "video" ? idx : -1))
                        .filter((idx) => idx >= 0);
            ...
            <div className="max-w-6xl mx-auto space-y-6">
              <div className="text-center space-y-2 mb-6">
                ...
                <div className="flex gap-4 justify-center text-sm">
                  <span className="text-tertiary font-medium">{getImageCount()} images</span>
                  <span className="text-tertiary font-medium">{getVideoCount()} videos</span>
                </div>
              </div>
          ...
          {currentStep === 6 && (
            <div className="max-w-6xl mx-auto space-y-6">
              <div className="text-center space-y-2 mb-6">
                ...
                <p className="text-muted-foreground">View the video and download it</p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                  ...
                  {mediaItems.some((item) => item.type === "titleCard") && (
                    <TitleCardCustomizer settings={titleCardSettings} onChange={setTitleCardSettings} />
                  )}
                </div>
          ...
          {currentStep > 1 && (
            <div className="flex justify-between max-w-6xl mx-auto mt-8">
              ...
              {currentStep < 6 && (
                <Button onClick={handleNext} size="lg" disabled={!canGoNext()} className="gap-2">
                  Next
